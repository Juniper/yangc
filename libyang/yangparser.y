...
%token Y_NONE			/* Does not accept an argument */
%token Y_NUMBER			/* Number (integer) */
%token Y_ORDERED		/* 'system' or 'user' */
%token Y_RANGE			/* Range (max or min .. max) */
%token Y_REGEX			/* Regular expression */
%token Y_STATUS			/* Status: current, deprecated, or obsolete */
%token Y_STRING			/* Simple string */
%token Y_TARGET			/* Target hierarchy/path */
%token Y_XPATH			/* XPath expression */

%token Y_LAST			/* Marker for last YANG token */
 ...
 * and we get a third one from the YANG xpath argument, and
 * we get one more from the "range" argument.
 */
%expect 4
...
#define yydebug yangYyDebug	/* Make debug flag parser specific */
...
block_stmt_list :
	...
	| block_stmt_list main_block_stmt
		{
		    ALL_KEYWORDS_ON();
		    $$ = NULL;
		}
	...
	yang_stmt_name
		{
	     ...
	     yang_stmt_value_optional
		{
		    slax_data->sd_ytype = 0;
		    slax_data->sd_flags &= ~SDF_STRING;
		    ALL_KEYWORDS_ON();
		    yangStmtCheckArgument(slax_data, $3);
		    $$ = NULL;
		}
	    yang_stmt_block
		{
		    ALL_KEYWORDS_ON();
		    yangStmtClose(slax_data, $1->ss_token);
		    $$ = STACK_CLEAR($1);
		}
	;

yang_stmt_name :
	yang_stmt_name_value
		{
		    yangStmtOpen(slax_data, $1->ss_token);
		    $$ = $1;
		}
	;
...
yang_stmt_value_optional :
	Y_BOOLEAN
		{ $$ = NULL; }
	| Y_BOOLEAN yang_argument_boolean_value
		{ $$ = $2; }

	| Y_DEVIATE
		{ $$ = NULL; }
	| Y_DEVIATE yang_argument_string_value
		{ $$ = $2; }

	| Y_IDENT
		{ $$ = NULL; }
	| Y_IDENT yang_argument_string_value
		{ $$ = $2; }
	...
	| Y_NUMBER
		{ $$ = NULL; }
	| Y_NUMBER yang_argument_string_value
		{ $$ = $2; }

	| Y_ORDERED
		{ $$ = NULL; }
	| Y_ORDERED yang_argument_string_value
		{ $$ = $2; }

	| Y_RANGE
		{ $$ = NULL; }
	| Y_RANGE yang_string_token
		{ $$ = $2; }
	| Y_RANGE yang_argument_xpath L_DOTDOT yang_argument_xpath
		{
		    slax_string_t *tmp, *ssp;
		    tmp = yangConcatValues(slax_data, $2, $3, TRUE);
		    ssp = yangConcatValues(slax_data, tmp, $4, TRUE);
		    yangStmtSetArgument(slax_data, ssp, FALSE);

		    $1 = $2 = $3 = $4 = NULL; /* Already freed */
		    $$ = ssp;
		}

	| Y_REGEX
		{ $$ = NULL; }
	| Y_REGEX yang_argument_string_value
		{ $$ = $2; }

	| Y_STATUS
		{ $$ = NULL; }
	| Y_STATUS yang_argument_string_value
		{ $$ = $2; }

	| Y_STRING
		{ $$ = NULL; }
	| Y_STRING yang_argument_string
		{
		    yangStmtSetArgument(slax_data, $2, FALSE);
		    $$ = $2;
		}

	| Y_TARGET
		{ $$ = NULL; }
	| Y_TARGET yang_argument_xpath_value
		{ $$ = $2; }

	| Y_XPATH
		{ $$ = NULL; }
	| Y_XPATH yang_argument_xpath_value
		{ $$ = $2; }
	;

yang_argument_boolean_value :
	yang_argument_string
		{
		    yangStmtSetArgument(slax_data, $1, FALSE);
		    $$ = $1;
		}
	;

yang_argument_string_value :
	yang_argument_string
		{
		    yangStmtSetArgument(slax_data, $1, FALSE);
		    $$ = $1;
...
yang_argument_string :
	...
	| yang_argument_string yang_string_item
		{
		    slax_string_t *ssp;
		    ssp = yangConcatValues(slax_data, $1, $2, TRUE);
		    $1 = $2 = NULL;
		    $$ = ssp;
		}

	| yang_argument_string L_PLUS yang_string_item
		{
		    slax_string_t *ssp;
		    ssp = yangConcatValues(slax_data, $1, $3, FALSE);
		    $1 = $3 = NULL;
		    $$ = ssp;
		}
	;
...
yang_stmt_block :
	L_EOS
		{ $$ = NULL; }
	| L_OBRACE yang_stmt_list L_CBRACE
		{ $$ = NULL; }
	;
